...
- **Verification:** After substantive edits, follow **`.cursor/rules/testing-verification.mdc`**: **`php artisan test`**, **`php artisan routes:smoke`**, **`npm run build`** when front-end changes, **`php artisan migrate:fresh --seed`** when migrations/seeders change (development DB only).
