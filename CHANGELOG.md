...
- **Admin · shop settings (Inici):** **General** only has **max manual** featured; **max tendència (stock baix)** sits in the **Stock baix** block (before **Sobrestock**); **max tendència** for sobrestock at **end** of **Sobrestock** block (label **without** «(sobrestock)» in ca/es). New i18n **`admin.settings.section_general`**.
