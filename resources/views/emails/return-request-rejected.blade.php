...
@section('title')
    {{ __('mail.return_request_rejected.subject', ['id' => $rma->order_id]) }}
@endsection

@section('content')
    <p>{{ __('mail.return_request_rejected.greeting') }}</p>
    <p>{{ __('mail.return_request_rejected.body', ['id' => $rma->order_id]) }}</p>
    @if($rma->admin_notes)
