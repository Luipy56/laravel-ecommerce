...
- **Git — before you change anything:** From repo root run **`./scripts/git-sync-agent-branch.sh`** before editing **`CLOSED-*.md`** or running **`move-agent-task-to-done.sh`**. See **`.cursor/rules/git-agent-branch-workflow.mdc`**.
- Do not edit **`app/`** or **`resources/js/`**.
- Do not create new tasks.
- **GitHub:** final comment on **#NN**, remove **`agent:testing`** / **`agent:wip`**, close issue if fully delivered (see **`docs/agent-loop.md`**) when applicable.
