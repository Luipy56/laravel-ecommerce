...
- **Verification:** After substantive edits, follow **`.cursor/rules/testing-verification.mdc`**: **`php artisan test`**, **`php artisan routes:smoke`**, **`npm run build`** when front-end changes, **`php artisan migrate:fresh --seed`** when migrations/seeders change (development DB only). **App version:** shippable work should bump root **`package.json`** patch semver before push so **`footer.version`** stays accurate (**`.cursor/rules/app-version-cadence.mdc`**).
