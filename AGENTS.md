...
- **Verification:** After substantive edits, follow **`.cursor/rules/testing-verification.mdc`**: **`php artisan test`**, **`php artisan routes:smoke`**, **`npm run build`** when front-end changes, **`php artisan migrate:fresh --seed`** when migrations/seeders change (development DB only). **App version:** every agent task that commits must bump root **`package.json`** patch semver before push (**`.cursor/rules/agent-task-version-bump.mdc`**, **`app-version-cadence.mdc`**).
...
Before reporting that a fix is complete:
